...
{% set show_customer_specialorders = false %}        {# Displays Customer Special Order information at the bottom of receipts #}
...
<div style="background-color:white; position:relative; bottom:30px;height:40px;width:30%;"><p>Totaal aantal items: {{ transaction_item_count }}</p>
</div>
...
<strong> SWIFT BIC : </strong> KREDBEBB | <strong>  IBAN: </strong>BE65 7380-0639-0096 | <strong> KBC BANK: </strong> 7380-0639-0096
							  {# Bedankt{% if Sale.Customer %} {{Sale.Customer.firstName}} {{Sale.Customer.lastName}}{% endif %}! #} 
						{% endif %}
					</p> {% else %} 
   {% if Sale.Customer %}  <div> <div style="float:left; width: 70%;"> Naam en Handtekening Voor Ontvangst :     </div>  <div> DATUM :  </div> </div> {% endif %}
				{% endif %}
			</div>
...
{% macro store_receipt(Sale,parameters,options,Payment) %}
	<div class="store">
		...
		{% else %}
	<div class="thbckcolor">	<h2 class="paymentTitle">Betalingen</h2> 
		...
		<p class="barcodeContainer">
			<img height="50" width="250" class="barcode" src="/barcode.php?type=receipt&number={{Sale.ticketNumber}}"></p>
...
<div style="float:right; width:40%;position:relative; top:-50px;right:100px;">
		{% if Sale.Customer %}
			{% if Sale.Customer.company|strlen > 0 %}
				<span class="receiptCompanyNameField"><strong class="receiptCompanyNameLabel"></strong><span id="receiptCompanyName">{{Sale.Customer.company}}</span><br /></span>
			{% endif %}

			{% if not options.company_name_override or not Sale.Customer.company|strlen > 0 %}
			{#	<span class="receiptCustomerNameField"><strong class="receiptCustomerNameLabel"></strong><span id="receiptCustomerName">{{Sale.Customer.firstName}} {{Sale.Customer.lastName}}</span><br /></span>
		#} 	{% endif %} 
			...
			{% if not options.show_customer_name_only %}
				...
				<span id="receiptPhonesContainer" class="indent">
					{% for Phone in Sale.Customer.Contact.Phones.ContactPhone %}
						<span data-automation="receiptPhoneNumber"> <strong style="padding-right:26px;">Tel </strong><strong >: </strong> {{Phone.number}}</span><br />
					{% endfor %}

					{% for Email in Sale.Customer.Contact.Emails.ContactEmail %}
<div style="display:none;">
						<span class="receiptEmailLabel"></span><span id="receiptEmail">{{Email.address}} ({{Email.useType}})</span><br />
			...
			{% if options.show_customer_notes %}
				{% if Sale.Customer.Note.note|strlen > 0 %}
					<span class="receiptCustomerNoteField"><strong class="receiptCustomerNoteLabel">Opmerking: </strong>{{ Sale.Customer.Note.note|noteformat|raw }}<br /></span>
				{% endif %}
			{% endif %}
		{% endif %}
	</p>
		...
		{% else %}
			<span class="receiptTicketIdField">
				<strong class="receiptTicketIdLabel">
					{% if options.sale_id_instead_of_ticket_number %}Factuurnummer: {% else %}Ticket: {% endif %}</strong>
...
<strong >: </strong>
		...
		{% if Sale.Register and not options.hide_register_name %}
			<span class="receiptRegisterNameField"><strong class="receiptRegisterNameLabel">Kassa: </strong><span id="receiptRegisterName">{{Sale.Register.name}}</span><br /></span>
		{% endif %}

		{% if Sale.Employee and not options.hide_employee_name %}
			<span class="receiptEmployeeNameField"><strong class="receiptEmployeeNameLabel">Medewerker: </strong><span id="receiptEmployeeName">{{Sale.Employee.firstName}}</span><br /></span>
		{% endif %}
		...
		{% if options.show_msrp == true and not parameters.gift_receipt %}
			...
			{% for price in Line.Item.Prices.ItemPrice if not msrp_printed %}
				{% if price.useType == "MSRP" and price.amount != "0"%}
					<td class="custom_field">{{ price.amount }}</td>
...
<td data-automation="lineItemQuantity" class="quantity">
  ...
  {{ divide(Line.calcSubtotal, Line.unitQuantity) | round (2)}} {#{{ divide(Line.displayableSubtotal, Line.unitQuantity) }}#}
				{% else %}
...
{% macro receipt(Sale,parameters,store_copy,options) %}
	{% if Sale.SaleLines %}
		<table class="sale lines zebra">
			<tr class="thbckcolor">
				<th class="description">Product Naam/Beschrijvening</th>
...
{% for Tax in Sale.TaxClassTotals.Tax %}
				...
				<td data-automation="receiptSaleTotalsTaxValue" class="amount">		{{Sale.displayableSubtotal |money}}</td>
						{% endif %}
						{% if Tax.taxname2 and Tax.rate2 > 0 %}
							<td data-automation="receiptSaleTotalsTaxName" width="100%">{{Tax.taxname2}} ({{Tax.subtotal2|money}} @ {{Tax.rate2}}%)</td><td data-automation="receiptSaleTotalsTaxValue" class="amount">{{Tax.amount2|money}}</td>
						{% endif %}
					{% endfor %}
					...
					{% for Tax in Sale.TaxClassTotals.Tax %}
						{% if Tax.taxname and Tax.rate > 0 %}
							<tr><td data-automation="receiptSaleTotalsTaxName" width="100%">BTW  {{Tax.rate}}%</td><td data-automation="receiptSaleTotalsTaxValue" class="amount">{{Tax.amount|money}} </td></tr>
						{% endif %}
						{% if Tax.taxname2 and Tax.rate2 > 0 %}
						...
						{% endif %}
					{% endfor %}
	{# 				<tr><td width="100%" height="10px"></td>
<td width="100%">Totaal belasting</td><td id="receiptSaleTotalsTax" class="amount">{{Sale.taxTotal|money}}</td> 
                    ...
                    {% if roundedTotal != Sale.calcTotal %}
                        {% if Sale.completed == "false" and Sale.quoteID > 0 %}
                            <tr class="total"><td>TOTAAL </td><td id="receiptSaleTotalsTotal" style="border-top: 
black 2px solid; height:40px;"  class="amount">{{Sale.calcTotal|money}}</td></tr>
                        {% else %}
                     {#       <tr class="total"><td>Totaal</td><td id="receiptSaleTotalsTotal" style="border-top: 
black 2px solid; height:40px;" class="amount">{{Sale.calcTotal|money}}</td></tr> #}

                            <tr class="total" style="height:30px;"><td>TOTAAL</td><td style="border-top: 
#97defa 1px solid; " id="receiptSaleTotalsRoundedTotal" class="amount" style="border-top: 
black 2px solid; height:40px;" >{{roundedTotal|money}}</td></tr>
                        {% endif %}
                    {% else %}
                        <tr class="total"><td>TOTAAL</td><td id="receiptSaleTotalsTotal" style="border-top: 
black 2px solid; height:40px;"  class="amount">{{Sale.calcTotal|money}}</td></tr>
	...
	{% if Sale.completed == 'true' and not parameters.gift_receipt %}
		{% if Sale.SalePayments %}
			<div class="thbckcolor" style="display:none;"><h2 class="paymentTitle">Betalingen</h2> 
...
{% macro header(Sale,options) %}
	<div class="receiptHeader">
		...
		{% if Sale.Shop.ReceiptSetup.logo|strlen > 0 and not logo_printed %}
			<img src="https://cdn.webshopapp.com/shops/332193/files/389148879/logo1.jpg" width="{{ options.logo_width }}" height="{{ options.logo_height }}" class="logo">
