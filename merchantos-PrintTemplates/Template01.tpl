{#
    ***Begin Custom Options***
    ...
    {# Layout Adjustments #}
    {% set print_layout = parameters.print_layout == "true" ? true : false %} {# Improves receipt layout for large
    display/paper size (A4/Letter/Email) #}
    {% set chrome_right_margin_fix = false %} {# Fixes a potential issue where the right side of receipts are cut off in
    Chrome #}
    {% set firefox_margin_fix = false %} {# Fixes issue with margins cutting off when printing on a letter printer on a Mac
    #}
    
    {# Sale #}
    {% set hide_register_name = true %} {# Hide the Register Name on Sales #}
    {% set hide_employee_name = true %} {# Hide the Employee Name on Sales #}
    ...
    {% set invoice_as_title = false %} {# If print_layout is true, "Invoice" will be displayed instead of "Sales Receipt" on
    A4/Letter/Email formats #}
    {% set workorders_as_title = false %} {# Changes the receipt title to "Work Orders" if there is no Salesline items and 1
    or more workorders #}
    {% set quote_id_prefix = "" %} {# Adds a string of text as a prefix for the Quote ID. Ex: "Q-". To be used when
    "sale_id_instead_of_ticket_number" is true #}
    {% set sale_id_prefix = "" %} {# Adds a string of text as a prefix for the Sales ID. Ex: "S-". To be used when
    "sale_id_instead_of_ticket_number" is true #}
    ...
    {% set discounted_line_items = true %} {# Strikes out the original price and displays the discounted price on each Sale
    Line #}
    ...
    {% set show_thank_you = true %} {# Displays "Bedankt <Customer Name>!" above bottom barcode #}
        ...
        {% set quote_to_invoice = false %} {# Changes Offerte wording to Factuur in Sales and in Sale Quotes (does not apply
        to Work Order Quotes) #}
        ...
        {# Customer Account #}
        {% set show_credit_account_signature = false %} {# Prints Store Copy with signature line on accounts that use an
        Account Credit (not Deposit) #}
        {% set show_customer_layaways = false %} {# Displays Customer Layaway information at the bottom of receipts #}
        {% set show_customer_specialorders = false %} {# Displays Customer Special Order information at the bottom of
        receipts #}
        {% set show_customer_workorders = false %} {# Displays Customer Work Order information at the bottom of receipts #}
        {% set show_customer_credit_account = false %} {# Displays Customer Credit Account information at the bottom of
        receipts #}
    
        {# Logos #}
        {% set logo_width = '225px' %} {# Default width is 225px. #}
        {% set logo_height = '' %} {# Default height is 55px. #}
        {% set multi_shop_logos = false %} {# Allows multiple logos to be added for separate locations when used with
        options below #}
    
        {#
        Use the following shop_logo_array to enter all of your locations and the link to the logo image that you have
        uploaded to the internet.
        Enter your EXACT shop name (Case Sensitive!) in the Quotes after the "name": entry and then enter the URL to your
        logo after the "logo": entry.
        Be sure to set the multi_shop_logos setting above to true in order to have these logos take effect!
        #}
    
        {% set shop_logo_array =
        {
        0:{"name":"Galesco Group", "logo_url":"https://cdn.webshopapp.com/shops/332193/files/389148879/logo1.jpg"},
        1:{"name":"", "logo_url":""},
        2:{"name":"", "logo_url":""},
        3:{"name":"", "logo_url":""},
        4:{"name":"", "logo_url":""},
        ...
        ***End Custom Options***
        ...
        margin: 0 auto;
        padding: 1px;
        <!-- You need this to make the printer behave -->
        ...
        * {
        ...
        .adreess{
        background-image: url("https://cdn.webshopapp.com/shops/332193/files/389149490/adress1.jpg");
        float: right;
        font-size: 10pt;
        text-align: center;
        font: normal 10pt “Helvetica Neue”, Helvetica, Arial, sans-serif;
        ...
        .zebra tr {border: #97defa 1px dotted;}
        .receiptHeader {
        /*text-align: center;*/
        ...
        padding-top: 10px;
        padding-bottom: 10px;
        background-image: url(https://cdn.webshopapp.com/shops/332193/files/390425359/g.png);
        background-repeat: no-repeat;
        height: 1200px;
    ...
    text-align: center; }
        ...
        {% else %}
        {% if SalePayment.archived == 'false' and SalePayment.MetaData.ReceiptData.requires_receipt_signature|CompBool ==
        true %}
        {{ _self.store_receipt(Sale,parameters,_context,SalePayment) }}
        {% set page_loaded = true %}
        {% endif %}
        ...
        <div class="bgreceipt">
            <div class="adreess">
                <h1> Galesco Group <br></h1>
                <p> Bredabaan 27 <br>
                    2170 Merksem | Belgie <br>
                    Tel : 32 (0) 33759600 <br><br>
                    info@galesco.be <br>
                    www.galesco.be </p>
                <b>BTW:0478.039.952</b> <br>
            </div>
            ...
            <div style="background-color:white; position:relative; bottom:30px;height:40px;width:30%;">
                <p>Totaal aantal items: {{ transaction_item_count }}</p>
            </div>
    
            {% endif %}
    
            {% if Sale.quoteID and Sale.Quote.notes|strlen > 0 %}<p id="receiptQuoteNote" class="note quote">
                {{Sale.Quote.notes|noteformat|raw}}</p>{% endif %}
    
            {% if Sale.Shop.ReceiptSetup.generalMsg|strlen > 0 %}<p id="receiptNote" class="note">{{
                Sale.Shop.ReceiptSetup.generalMsg|noteformat|raw }}</p>{% endif %}
            ...
            <p class="barcodeContainer">
                <img id="barcodeImage" height="50" width="250" class="barcode"
                    src="/barcode.php?type=receipt&number={{Sale.ticketNumber}}&hide_text={{ not show_barcode_sku }}">
            </p>
            ...
            <p id="receiptThankYouNote" class="thankyou">
                ...
                <strong> SWIFT BIC : </strong> KREDBEBB | <strong> IBAN: </strong>BE65 7380-0639-0096 | <strong> KBC BANK:
                </strong> 7380-0639-0096
                {# Bedankt{% if Sale.Customer %} {{Sale.Customer.firstName}} {{Sale.Customer.lastName}}{% endif %}! #}
                {% endif %}
            </p> {% else %}
            {% if Sale.Customer %} <div>
                <div style="float:left; width: 70%;"> Naam en Handtekening Voor Ontvangst : </div>
                <div> DATUM : </div>
            </div> {% endif %}
            {% endif %}
        </div>
        ...
        <div class="store">
            ...
            {% if Sale.quoteID and Sale.Quote.notes|strlen > 0 %}<p class="note quote">{{Sale.Quote.notes|noteformat|raw}}
            </p>{% endif %}
        ...
        <div class='line_description'>
            {% autoescape true %}{{ Line.Item.description|nl2br }}{% if Line.tax == 'false' or (Line.calcTax1 == 0 and
            Line.calcTax2 == 0) %}*{% endif %}{% endautoescape %}
        </div>
        {% endif %}
        {% if Line.Note %}
        <div class='line_note'>
        ...
        <div style="float:right; width:40%;position:relative; top:-50px;right:100px;">
            {% if Sale.Customer %}
            {% if Sale.Customer.company|strlen > 0 %}
            <span class="receiptCompanyNameField"><strong class="receiptCompanyNameLabel"></strong><span
                    id="receiptCompanyName">{{Sale.Customer.company}}</span><br /></span>
            {% endif %}
    
            {% if not options.company_name_override or not Sale.Customer.company|strlen > 0 %}
            {# <span class="receiptCustomerNameField"><strong class="receiptCustomerNameLabel"></strong><span
                    id="receiptCustomerName">{{Sale.Customer.firstName}} {{Sale.Customer.lastName}}</span><br /></span>
            #} {% endif %}
    
            {% if not options.show_customer_name_only %}
            {% if options.show_full_customer_address %}
            <span class="receiptCustomerAddressField">
                <strong class="receiptCustomerAddressLabel"></strong>
                {{ _self.address(Sale.Customer.Contact,'<br>',options) }}
            </span>
            {% endif %}
    
            <span id="receiptPhonesContainer" class="indent">
                {% for Phone in Sale.Customer.Contact.Phones.ContactPhone %}
                <span data-automation="receiptPhoneNumber"> <strong style="padding-right:26px;">Tel </strong><strong>:
                    </strong> {{Phone.number}}</span><br />
                {% endfor %}
    
                {% for Email in Sale.Customer.Contact.Emails.ContactEmail %}
                <div style="display:none;">
                    <span class="receiptEmailLabel"></span><span id="receiptEmail">{{Email.address}}
                        ({{Email.useType}})</span><br />
            ...
            <span class="receiptCustomerVatField">
                <strong class="receiptCustomerVatLabel">BTW-nr: </strong>
            ...
            <span class="receiptCustomerNoteField"><strong class="receiptCustomerNoteLabel">Opmerking: </strong>{{
                Sale.Customer.Note.note|noteformat|raw }}<br /></span>
            {% endif %}
            {% endif %}
            {% endif %}
            </p>
        ...
        <div style="float:left;width: 100%;">
            ...
            <h1 class="receiptTypeTitle" style="font-size:20px; padding-bottom:20px; text-align:left;">
                {% if Sale.calcTotal >= 0 %}
                ...
                <span class="hide-on-print">
                    ...
                    Werkbonnen
                    {% else %}
                    {% if parameters.gift_receipt %}LEVERINGSBON{% else %}FACTUUR{% endif %}
                    {% endif %}
                    {% if options.invoice_as_title and options.print_layout %}
                </span>
            ...
            <p id="receiptInfo" class="details">
                ...
                <span class="receiptQuoteIdField">
                    <span class="receiptQuoteIdLabel" style="font-weight: bold;">
                        {% if options.quote_to_invoice %}
                        Factuurnr #:
                        {% else %}
                        Offertenummer :
                ...
                <span class="receiptTicketIdField">
                    <strong class="receiptTicketIdLabel">
                        {% if options.sale_id_instead_of_ticket_number %}Factuurnummer: {% else %}Ticket: {% endif
                        %}</strong>
                    <span id="receiptTicketId">
                        {% if options.sale_id_instead_of_ticket_number %}
                        {{options.sale_id_prefix}}
                ...
                <span class="companyRegistrationNumberField">
                    <strong class="companyRegistrationNumberLabel">Handelsregisternr: </strong>
                ...
                <span class="receiptCustomerCompanyVatField">
                    <strong style="padding-right:12px;" class="receiptCustomerCompanyVatLabel">Klantnummer</strong>
                    <strong>: </strong>
                ...
                <span class="receiptRegisterNameField"><strong class="receiptRegisterNameLabel">Kassa: </strong><span
                        id="receiptRegisterName">{{Sale.Register.name}}</span><br /></span>
                {% endif %}
    
                {% if Sale.Employee and not options.hide_employee_name %}
                <span class="receiptEmployeeNameField"><strong class="receiptEmployeeNameLabel">Medewerker: </strong><span
                        id="receiptEmployeeName">{{Sale.Employee.firstName}}</span><br /></span>
                {% endif %}
    
            <div>
                <div>
                    <p class="date" id="receiptDateTime">
                        <strong class="receiptEmployeeNameLabel">Date: </strong>
                        {% if Sale.timeStamp %}
                        {{Sale.timeStamp|correcttimezone|date(getDateFormat())}}
                        {% else %}
                        {{"now"|date(getDateFormat())}}
                        {% endif %}
                    </p>
                </div>
            ...
            <script>    
                try {
                    ...
                    //var testObj2 = "{{Line.Item.Prices.ItemPrice }}"
                    var unitPrice = "{{Line.unitPrice}}" //Fiyat al
                    var unitQuantity = "{{Line.unitQuantity}}" // Miktar Al
                    var productDesc = "{{Line.Item.description }}" // Urun adini al
                    //var boxQuantity = productDesc.split(" X (")[0].match(/\d+/)[0] //Urun adından kutu ici miktari cikar
                    // match(/\d+/)[0] : bir string deki sayilari bulur
                    // Yeni Sistem İçin AltSatır Örn:Neutral Afwasmiddel Hand Dishwash [6x500 ml]
                    if (productDesc.indexOf("[")>0 && productDesc.indexOf("]")>0) {
                        var boxQuantity = productDesc.split("[")[1].split("x")[0].match(/\d+/)[0]
                    
                    } else {
                        var boxQuantity = 1
                    }
                    var ItemInBoxUnitPrice = (unitPrice / boxQuantity).toFixed(2); //Birim fiyati hesapla
                    var aantal = unitQuantity * boxQuantity // aantal i hesapla 
                    
                    invoiceItems.push({title:productDesc,rowId:"#row_" + itemID,boxQuantity,ItemInBoxUnitPrice,aantal });
                    
                    window.onload = function () //jquery metodlarını sayfa yuklenince calistir
                    {
                        invoiceItems.map(function(item, index)
                         { 
                             var numRow=index+1;//Eğer rakam tek haneli ise 2ye tamamlamak için
                             numRow = numRow<10 ? "0"+numRow : numRow;
                            $(item.rowId + " .spnItemInBoxUnitPrice").text(item.ItemInBoxUnitPrice);
            ...
            <tr id="row_{{Line.Item.itemID }}">
                ...
                <td data-automation="lineItemDescription" class="description descriptionCol">
                    ...
                    {% elseif Line.calcLineDiscount < 0 %} <small>Korting: '{{ Line.Discount.name }}'
                        {{Line.calcLineDiscount|getinverse|money}}</small>
                        {% endif %}
                        {% endif %}
                </td>
    
                {% if options.show_custom_sku and Line.Item.customSku %}
                <td class="custom_field artikelnummerCol">{{ Line.Item.customSku }}</td>
                {% endif %}
                {% if options.show_manufacturer_sku and Line.Item.manufacturerSku %}
                <td class="custom_field artikelnummerFabrCol">{{ Line.Item.manufacturerSku }}</td>
                {% endif %}
                ...
                <td class="custom_field manufacturerSkuCol">{{ price.amount }}</td>
                {% set msrp_printed = true %}
                {% endif %}
                    {% endfor %}
                    {% if not msrp_printed %}
                    <td class="custom_field AdviesprijsCol">N.v.t.</td>
                    {% endif %}
                {% endif %}
    
                <td data-automation="lineItemQuantity" class="quantity AantalCol">
                    {% if options.per_line_subtotal and options.discounted_line_items and Line.calcLineDiscount != 0 and not
                    parameters.gift_receipt %}
                    <span class="strike">{{Line.unitQuantity}} x {{Line.displayableUnitPrice}}</span>
                    {% endif %}
                    <!-- {{Line.unitQuantity}} -->
                    <span class="spnAantal">-</span>
                </td>
                
                {% if not parameters.gift_receipt %}
                <td data-automation="lineItemPrice" class="amount UnitPrijsCol" style="text-align: center;">
                    <span class="spnItemInBoxUnitPrice">-</span>
                </td>
                <td data-automation="lineItemQuantity" class="quantity epCol" style="display:none;">
                    {% if options.per_line_subtotal and options.discounted_line_items and Line.calcLineDiscount != 0 and not
                    parameters.gift_receipt %}
                    <span class="strike">{{Line.unitQuantity}} x {{Line.displayableUnitPrice}}</span>
                    {% endif %}
    
                    {% if options.per_line_subtotal and not parameters.gift_receipt %}
                    {% if options.discounted_line_items %}
                    {{ divide(Line.calcSubtotal, Line.unitQuantity) | round (2)}} {#{{ divide(Line.displayableSubtotal,
                    Line.unitQuantity) }}#}
                    {% else %}
                    {{Line.displayableUnitPrice}}
                    {% endif %}
                    {% endif %}
                </td>
                
                <td data-automation="lineItemPrice" class="amount PrijsCol">
                    ...
                    <span class="strike">{{ Line.calcSubtotal }}</span><br />
                    {% else %}
                    {# <span class="strike">{{ multiply(Line.displayableUnitPrice, Line.unitQuantity) }}</span><br /> #}
                    {% endif %}
                    {% endif %}
                    {{ Line.calcSubtotal }} {# {{ Line.displayableSubtotal }} #}
                    {% endif %}
                </td>
            </tr>
            {% endmacro %}
            ...
            <table class="sale lines zebra">
                ...
                <tr class="thbckcolor">
                    ...
                    <th class="custom_field manufacturerSkuCol">Artikelnummer</th>
                    {% endif %}    
            ...
            <table class="saletotals totals">
                <tbody id="receiptSaleTotals">
                    ...
                    <tr>
                        ...
                        {% if Tax.taxname and Tax.rate > 0 %}
                        <tr>
                            <td data-automation="receiptSaleTotalsTaxName" width="100%">BTW {{Tax.rate}}%</td>
                            <td data-automation="receiptSaleTotalsTaxValue" class="amount">{{Tax.amount|money}} </td>
                        </tr>
                        {% endif %}
                        {% if Tax.taxname2 and Tax.rate2 > 0 %}
                        <tr>
    ...
    black 2px solid; height:40px;" class="amount">{{Sale.calcTotal|money}}</td>
                        ...
                        <tr class="total" style="height:30px;">
                            <td>TOTAAL</td>
                            <td style="border-top: 
    #97defa 1px solid; " id="receiptSaleTotalsRoundedTotal" class="amount" style="border-top: 
    black 2px solid; height:40px;">{{roundedTotal|money}}</td>
                        </tr>
                        {% endif %}
                        {% else %}
                        <tr class="total">
    ...
    black 2px solid; height:40px;" class="amount">{{Sale.calcTotal|money}}</td>
                        ...
                        <tr class="tip">
                            <td>Fooi</td>
                            <td id="receiptSaleTotalsTip" class="amount">{{Sale.calcTips|money}}</td>
                        </tr>
                        {% endif %}
                </tbody>
            </table>
            ...
            <div class="thbckcolor" style="display:none;">
                <h2 class="paymentTitle">Betalingen</h2>
                <table id="receiptPayments" class="payments">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td id="receiptPaymentsGiftCardBalance" class="amount">
                                {{Payment.CreditAccount.balance|getinverse|money}}</td>
                        </tr>
                        {% elseif Payment.amount < 0 and Sale.calcTotal < 0 %} <tr>
                            ...
                            {% elseif Payment.amount < 0 and Sale.calcTotal>= 0 %}
                                <tr>
                                    ...
                                    <td id="receiptPaymentsGiftCardValue" class="amount">{{Payment.amount|getinverse|money}}
                                    </td>
                                </tr>
                                <tr>
                                    <td class="label">Saldo</td>
                                ...
                                <!-- Customer Account -->
                                <tr>
                                    {% if Payment.amount < 0 %} <td class="label">Aanbetaling op rekening</td>
                                        <td id="receiptPaymentsCreditAccountDepositValue" class="amount">
                                            {{Payment.amount|getinverse|money}}</td>
                                        {% else %}
                                        <td class="label">Betalen op rekening</td>
                                        <td id="receiptPaymentsCreditAccountChargeValue" class="amount">
                                            {{Payment.amount|money}}</td>
                                        {% endif %}
                                </tr>
                                {% endif %}
                                {% endif %}
            ...
            %}
            {% if Sale.Customer.CreditAccount and Sale.Customer.CreditAccount.MetaData.creditBalanceOwed > 0 or
            Sale.Customer.CreditAccount.MetaData.extraDeposit > 0 %}
            ...
            {% endif %}
            {% endif %}
            {% if (not parameters.gift_receipt and not options.hide_notes_in_sale_receipt) or (parameters.gift_receipt and
            not options.hide_notes_in_gift_receipt) %}
            {{ _self.show_note(Sale.SaleNotes) }}
            {% endif %}
            {% endmacro %}
            ...
            <tr>
                ...
                <td class="label" width="100%">
                    ...
                    {% endif %}
                    {% if Payment.CCCharge.gatewayTransID|strlen > 0 and Payment.CCCharge.gatewayTransID|strlen < 48 %} <br>
            ...
            {% if options.workorders_as_title and Sale.SaleLines is empty and Sale.Customer.Workorders is defined and
            Sale.Shop.ReceiptSetup.workorderAgree|strlen > 0 %}
            <p>{{Sale.Shop.ReceiptSetup.workorderAgree|noteformat|raw}}</p>
            {% endif %}
            {% endmacro %}
            ...
            {% if Address.state|strlen > 0 %}{{Address.state}}{% endif %}{% if Address.zip|strlen > 0 or Address.city|strlen
            > 0 or Address.state|strlen > 0 %}{{delimiter}}{% endif %}
            ...
            <div class="receiptHeader">
                ...
                <img src="{{ shop.logo_url }}" width="{{ options.logo_width }}" height="{{ options.logo_height }}"
                    class="logo">
                ...
                <img src="https://cdn.webshopapp.com/shops/332193/files/389148879/logo1.jpg"
                    width="{{ options.logo_width }}" height="{{ options.logo_height }}" class="logo">
            ...
            <tr>
                <td>Contant betaald bedrag</td>
                <td>{{originalCashPayedTotalValue|money}}</td>
            </tr>
            {% if originalCashPayedTotalValue == roundedCashPayedTotalValue %}
            <tr>
                <td class="label">Contant</td>
                <td id="receiptPaymentsCash" class="amount">{{cashTotal|money}}</td>
            </tr>
            {% if (Sale.change|floatval) != roundedCashChange %}
            <tr>
                <td class="label">Wisselgeld</td>
                <td id="receiptPaymentsChange" class="amount">{{Sale.change|money}}</td>
            </tr>
            <tr>
                <td class="label"> Wisselgeld afgerond </td>
                <td id="receiptRoundedPaymentsChange" class="amount">{{roundedCashChange|money}}</td>
            </tr>
            {% else %}
            <tr>
                <td class="label">Wisselgeld</td>
            ...
            <tr>
                <td class="label">Totaal contant afgerond </td>
                <td id="receiptRoundedPaymentsCash" class="amount">{{roundedCashAmount|money}}</td>
            </tr>
            {% if (Sale.change|floatval) != roundedCashChange %}
            <tr>
                <td class="label">Wisselgeld </td>
